...
interface CardPrazoProps {
  dataFechamentoRevisaoGestor?: string | null;
}

export const CardPrazo = ({ dataFechamentoRevisaoGestor }: CardPrazoProps) => {
  // Calcular dias restantes
  const calcularDiasRestantes = () => {
    if (!dataFechamentoRevisaoGestor)
      return { dias: 0, dataFormatada: "Data não definida" };

    const hoje = new Date();
    const dataFim = new Date(dataFechamentoRevisaoGestor);
  ...
  return (
    <DashboardInfoCard title="Prazo" bgColor="bg-white" textColor="text-black">
          ...
          <span className="text-sm text-[#4F4F4F] leading-tight">
            {dataFechamentoRevisaoGestor ? (
              <>
                ...
                <br />
                das notas, no dia {dataFormatada}
              </>
            ) : (
              <>
                Data de fechamento
